...
export async function loadEstadisticas({ desde, hasta }) {
  try {
    logger.debug('📊 Cargando estadísticas:', { desde, hasta });

    // Validación: no enviar null a RPC
    if (!desde || !hasta) {
      logger.warn('⚠️ Fechas vacías, retornando datos por defecto');
      return EMPTY_STATS;
    }
    ...
    try {
      const { data: casesInRange, error: casesErr } = await withRetry(() =>
        supabase
          .from('cases')
          .select('student_id, students(first_name, last_name, rut)')
      ...
      logger.warn('No se pudo obtener lista de reincidentes:', e);
    }
    ...
    const promedioSeguimientos = pickSingle(
      promSegRes.data,
      EMPTY_STATS.promedioSeguimientos,
    );
    const promedioDiasPrimerSeguimiento = pickSingle(
      promDiasRes.data,
      EMPTY_STATS.promedioDiasPrimerSeguimiento,
    );

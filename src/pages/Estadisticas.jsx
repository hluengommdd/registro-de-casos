...
export default function Estadisticas() {
  ...
  return (
    <div className="container space-y-8 print-container pb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {(() => {
          ...
          return kpiItems.map((item, idx) => (
            <div key={item.label} className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex items-center gap-4 transition-transform hover:scale-[1.02]">
              <div className={`w-12 h-12 rounded-full ${palette[idx % palette.length]} bg-opacity-10 flex items-center justify-center text-xl shadow-sm border border-white`}>
                {item.icon}
              </div>
              <div className="flex-1">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">{item.label}</p>
                <p className="text-2xl font-extrabold text-slate-800 mt-1">{item.value} <span className="text-sm font-medium text-slate-400">{item.suffix || ''}</span></p>
              </div>
            </div>
          ))
        })()}
      ...
      {/* KPI DIRECTIVOS */}
      <h2 className="text-xl font-bold text-slate-800 -mb-2">Indicadores de Gestión</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Cumplimiento de plazos</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className={`text-3xl font-extrabold ${cumplimientoPlazo >= 90 ? 'text-emerald-600' : cumplimientoPlazo >= 70 ? 'text-amber-500' : 'text-red-500'}`}>
              {cumplimientoPlazo}%
            </span>
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">
            {plazos.fuera_plazo} casos fuera de plazo / {plazos.total_plazos} total
          </p>
        </div>

        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Reincidencia</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-3xl font-extrabold text-brand-600">
              {reincidencia}
            </span>
            <span className="text-sm font-medium text-slate-400">estudiantes</span>
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Mayor carga</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Nivel más conflictivo</p>
          <div className="mt-2 text-3xl font-extrabold text-slate-800">
            {mayorNivel.total ?? 0}
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Promedio seguimientos</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-3xl font-extrabold text-slate-800">
              {Number(promedioSeguimientos.promedio ?? 0).toFixed(1)}
            </span>
            <span className="text-sm font-medium text-slate-400">acciones/caso</span>
          </div>
        </div>

        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Días primer seguimiento</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-3xl font-extrabold text-slate-800">
              {Number(promedioDiasPrimerSeguimiento.promedio_dias ?? 0).toFixed(1)}
            </span>
            <span className="text-sm font-medium text-slate-400">días</span>
          </div>
          <p className="text-xs text-slate-400 font-medium mt-1">Promedio desde incidente</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <table className="w-full text-sm">
                <thead className="text-left text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200 bg-slate-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 rounded-tr-lg text-right">Cantidad de Casos</th>
                  </tr>
                </thead>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h3 className="font-bold text-slate-800 mb-4">Evolución de Casos por Mes</h3>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h3 className="font-bold text-slate-800 mb-4">Casos por Tipificación</h3>
          <div className="flex flex-wrap items-center gap-3 text-xs font-medium text-slate-600 mb-4">
            ...
            {Object.entries(TIPOS_COLORS).map(([name, color]) => (
              <div key={name} className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-slate-50 border border-slate-100">
            ...
            <PieChart>
              ...
              >
                {dataTipo.map((entry, i) => (
                  <Cell key={i} fill={TIPOS_COLORS[entry.name] || COLORS[i % COLORS.length]} stroke="transparent" />
                ))}
              </Pie>
              <Tooltip
                contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 col-span-2">
          <h3 className="font-bold text-slate-800 mb-4">Distribución por Curso</h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={dataCursos} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              <XAxis dataKey="curso" stroke="#64748b" fontSize={11} tickLine={false} axisLine={false} />
              <YAxis stroke="#64748b" fontSize={11} tickLine={false} axisLine={false} />
              <Tooltip
                cursor={{ fill: '#f1f5f9' }}
                contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}

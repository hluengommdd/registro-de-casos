...
export default function SeguimientoPage({
  ...
  if (loadingCaso) {
    return <div className="p-4 sm:p-6 text-gray-500">Cargando caso…</div>;
  }

  if (!caso) {
    return (
      <div className="p-4 sm:p-6 text-red-600">No se pudo cargar el caso.</div>
    );
  }
  ...
  return (
    <div
      className={`${embedded ? 'w-full' : 'max-w-5xl mx-auto'} p-4 sm:p-6 space-y-6`}
    >
      {/* HEADER */}
      <div className="flex justify-between items-center rounded-2xl glass-panel p-6 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm font-medium text-slate-500 mt-1 hidden sm:flex items-center gap-3">
                ...
                <span
                  className={`px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wide border ${esCerrado ? 'bg-slate-100 text-slate-600 border-slate-200' : 'bg-emerald-50 text-emerald-600 border-emerald-200'}`}
                >
                  {caso.fields.Estado}
                </span>
              </p>
        ...
        <div className="flex items-center gap-3 z-20">
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  const { default: InformeCasoDocument } =
                    await import('../components/InformeCasoDocument.jsx');
                  const { listEvidenceByFollowup } =
                    await import('../api/evidence');
                  ...
                  const seguimientosConEvidencias = await Promise.all(
                    (seguimientos || []).map(async (seg) => {
                      try {
                        ...
                        logger.warn(
                          'No se pudieron cargar evidencias para seguimiento',
                          seg.id,
                          e,
                        );
          ...
          {!esCerrado && !soloLectura && !hideNewAction && (
            <button
              onClick={() => {
                if (typeof setExternalMostrarForm === 'function')
                  setExternalMostrarForm(true);
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <div className="text-base font-black text-slate-800">
                  {getStudentName(caso.fields.Estudiante_Responsable, 'N/A')}
                </div>
                <div className="text-sm font-medium text-brand-600 mt-1">
                  Curso:{' '}
                  <span className="text-slate-600">
                    {caso.fields.Curso_Incidente || 'N/A'}
                  </span>
                </div>
              </div>
      ...
      {/* MODAL */}
      {!soloLectura &&
        (typeof externalMostrarForm === 'boolean'
          ? externalMostrarForm
          : mostrarForm) && (

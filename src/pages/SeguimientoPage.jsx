...
export default function SeguimientoPage({
  ...
  const grouped = useMemo(() => {
    ...
    for (const stage of Object.keys(acc)) {
      acc[stage].sort((a, b) =>
        String(a.action_at || a.fields?.Fecha || '').localeCompare(
          String(b.action_at || b.fields?.Fecha || ''),
        ),
      );
    }
    return acc;
  ...
  const completedStageKeys = useMemo(() => {
    ...
    for (const s of seguimientos || []) {
      const stage = s.fields?.Etapa_Debido_Proceso || s.process_stage;
      const status = s.fields?.Estado_Etapa || s.stage_status;

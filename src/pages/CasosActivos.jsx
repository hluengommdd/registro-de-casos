  ...
  useEffect(() => {
    async function cargar() {
      try {
        ...
        activos.sort(
          (a, b) =>
            parseLocalDate(a.fields?.Fecha_Incidente) -
            parseLocalDate(b.fields?.Fecha_Incidente),
        );
  ...
  return (
    <div className="h-full">
      <div className="flex flex-col lg:flex-row gap-6 h-full p-2 overflow-hidden">
        {/* LISTA IZQUIERDA */}
        <div
          className={`w-full lg:w-2/5 flex flex-col gap-4 h-full ${selectedCaso ? 'hidden lg:flex' : 'flex'}`}
        >
          {/* Header List */}
          <div className="flex items-center justify-between px-2 shrink-0">
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                {casos.length} expedientes en curso
              </p>
            </div>
            <button
                ...
                casos.map((caso) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="pl-2">
                        <h4
                          className={`text-xs font-bold mb-1 line-clamp-1 ${isSelected ? 'text-slate-900' : 'text-slate-700'}`}
                        >
                          {getStudentName(caso.fields.Estudiante_Responsable, 'Estudiante sin nombre')}
                        </h4>
                      ...
                      <div className="mt-3 pl-2 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-bold ${isSelected ? 'bg-slate-100 text-slate-600' : 'bg-slate-100 text-slate-500'}`}
                          >
                        ...
                        <span
                          className={`text-[10px] font-semibold px-1.5 rounded ${isSelected ? 'text-slate-600 bg-slate-100/50' : 'text-slate-400'}`}
                        >
                          {caso.fields.Estado}
                        </span>
                      </div>

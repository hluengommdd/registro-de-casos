...
export default function CierreCasoPage() {
  ...
  const isClosed = useMemo(() => {
    const estado = (caso?.fields?.Estado || caso?._supabaseData?.status || '').toString().trim().toLowerCase();
  ...
  const alumnoNombre = useMemo(() => {
    if (!caso) return '';
    return getStudentName(caso.fields?.Estudiante_Responsable, 'Estudiante');
  }, [caso]);

  const completedStages = useMemo(() => {
    const set = new Set();
    for (const s of seguimientos || []) {
      const stage = s.process_stage || s.fields?.Etapa_Debido_Proceso;
      const status = s.stage_status || s.fields?.Estado_Etapa;
  ...
  useEffect(() => {
    const row = caso?._supabaseData;
    if (!row) return;
    setDescripcionCierre((prev) => (prev ? prev : row.final_resolution_text || ''));
    setMedida((prev) => (prev && prev !== 'Amonestación Escrita' ? prev : row.final_disciplinary_measure || 'Amonestación Escrita'));
    setResponsableNombre((prev) => (prev ? prev : row.closed_by_name || ''));
    setResponsableRol((prev) => (prev ? prev : row.closed_by_role || ''));
  }, [caso]);

export function getStudentName(value, fallback = 'N/A') {
  ...
  if (typeof value === 'object') {
    ...
    const first = value.first_name || value.firstName || value.Nombre || value.nombre || '';
    const last = value.last_name || value.lastName || value.Apellido || '';
    const full = `${first} ${last}`.trim();
    if (full) return full;

    // Other possible combined fields
    if (value.fullName && typeof value.fullName === 'string') return value.fullName;
    if (value.full_name && typeof value.full_name === 'string') return value.full_name;

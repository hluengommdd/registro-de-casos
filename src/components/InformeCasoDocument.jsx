...
export default function InformeCasoDocument({ caso, seguimientos = [] }) {
  ...
  const seguimientosOrdenados = [...seguimientos].sort((a, b) => {
    const dateA =
      a.fields?.Fecha_Seguimiento ||
      a.fields?.Fecha ||
      a._supabaseData?.action_date ||
      a._supabaseData?.created_at ||
      '';
    const dateB =
      b.fields?.Fecha_Seguimiento ||
      b.fields?.Fecha ||
      b._supabaseData?.action_date ||
      b._supabaseData?.created_at ||
      '';
        ...
        <View style={styles.section}>
          ...
          <View style={styles.infoBox}>
            ...
            <View style={styles.row}>
              ...
              <Text style={styles.value}>
                {caso?.fields?.Tipificacion_Conducta || 'N/A'}
              </Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.label}>Categoría:</Text>
              <Text style={styles.value}>
                {caso?.fields?.Categoria || 'N/A'}
              </Text>
            </View>
            <View style={styles.row}>
            ...
            <View style={styles.row}>
              ...
              <Text style={styles.value}>
                {caso?.fields?.Hora_Incidente || 'N/A'}
              </Text>
            </View>
            <View style={styles.row}>
        ...
        <View style={styles.section}>
          ...
          {seguimientosOrdenados.length === 0 ? (
            ...
            seguimientosOrdenados.map((seg, i) => {
              const fecha =
                seg.fields?.Fecha_Seguimiento || seg.fields?.Fecha || '—';
            ...
            <View style={styles.closureBox}>
              ...
              {ultimoSeguimiento && (
                <>
                  <Text style={styles.closureText}>
                    Última actuación:{' '}
                    {ultimoSeguimiento.fields?.Tipo_Accion || 'Cierre del caso'}
                  </Text>
                  <Text style={styles.closureText}>
                    {ultimoSeguimiento.fields?.Detalle ||
                      ultimoSeguimiento.fields?.Descripcion ||
                      ''}
                  </Text>
                </>
              )}

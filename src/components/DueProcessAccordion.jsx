...
function groupByStage(followups = []) {
  ...
  for (const [_k, arr] of map.entries()) {
    arr.sort((a, b) =>
      (a?.fields?.Fecha || '').localeCompare(b?.fields?.Fecha || ''),
    );
  }
  return map;
}
...
export default function DueProcessAccordion({
  ...
  const filteredFollowups = useMemo(() => {
    return (followups || []).filter((f) => {
      const ff = f?.fields || {};
      const text =
        `${ff.Detalle || ''} ${ff.Descripcion || ''} ${ff.Acciones || ''} ${ff.Observaciones || ''}`.toLowerCase();
              ...
              <div className="min-w-0 text-left">
                ...
                {isOpen && (
                  <div className="text-xs text-brand-600/80 mt-1 font-medium">
                    {count > 0
                      ? 'Desplegando acciones...'
                      : 'Sin acciones registradas'}
                  </div>
                )}
              </div>
              <span
                className={`text-slate-400 text-lg transition-transform duration-200 ${isOpen ? 'rotate-180 text-brand-500' : ''}`}
              >
                ▾
              </span>
            </button>
                  ...
                  <div className="mt-2 space-y-3">
                    {items.map((f) => {
                      ...
                      return (
                        <div
                          key={f.id}
                          className="border border-slate-200 rounded-lg p-3 bg-white shadow-sm hover:border-brand-200 transition-colors"
                        >

    ...
    <div id={`seg-${seg.id}`} className="relative pl-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 hover:border-brand-200 transition-colors">
        ...
        <p className="text-xs text-slate-500 mt-1">
          <strong>Fecha:</strong> {formatDate(seg.action_date)} ·{' '}
          <strong>Responsable:</strong> {seg.responsible || '—'}
        </p>

        {/* Plazo / Fecha_Plazo */}
        {(() => {
          const fechaPlazo = seg.due_date || null;
          if (!fechaPlazo)
            return (
          ...
          const diff = Math.ceil((due.getTime() - today.getTime()) / msDay);

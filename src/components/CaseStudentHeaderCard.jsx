      ...
      <div className="bg-slate-700 px-4 sm:px-5 py-3 sm:py-4 text-white">
        <div className="flex items-start gap-3 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-sm sm:text-base font-bold leading-snug truncate">{studentName}</div>
            <div className="text-xs sm:text-sm opacity-90">Curso: <span className="font-semibold">{course}</span></div>

            {/* Chips - responsive flex */}
            <div className="mt-2 sm:mt-3 flex flex-wrap gap-1 sm:gap-2">
              <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-semibold ${tipColor(tipificacion)}`}>
                {tipificacion}
              </span>

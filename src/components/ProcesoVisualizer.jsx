      ...
      <div className="md:hidden overflow-x-auto pt-6 pb-2 -mx-4 px-4 border-t border-dashed border-slate-200"> 
        <div className="flex flex-row gap-6 min-w-max"> 
          {stages.map((stageKey, index) => { 
            ...
            return ( 
              <div key={stageKey} className={`flex flex-col items-center text-center transition-all ${isCurrent ? '-mt-1' : ''} min-w-[80px]`}> 
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${circleClass}`}> 
                  {isCompleted ? '✓' : stageNum} 
                </div>

                <div className={`mt-3 text-xs font-semibold leading-tight line-clamp-3 max-w-[80px] ${isCurrent ? 'text-brand-700' : 'text-slate-600'}`}> 
                  {stageName} 
                </div>

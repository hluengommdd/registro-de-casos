...
if grep -q "Etapa_Debido_Proceso || 'Seguimiento'" "$ROOT_DIR/src/api/db.js"; then
...
if grep -q "import.*getCase.*from.*api/db" "$ROOT_DIR/src/components/CaseDetailPanel.jsx"; then
...
if grep -q "emitDataUpdated()" "$ROOT_DIR/src/components/CaseDetailPanel.jsx"; then
...
if grep -q "1000" "$ROOT_DIR/src/components/CaseDetailPanel.jsx"; then
...
if grep -q "estado.*toLowerCase.*en seguimiento" "$ROOT_DIR/src/pages/Seguimientos.jsx"; then
...
if grep -q "import.*onDataUpdated" "$ROOT_DIR/src/components/Sidebar.jsx"; then
...
if [ -f "$ROOT_DIR/src/utils/refreshBus.js" ]; then

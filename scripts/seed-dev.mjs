...
async function ensureConductCatalog() {
  ...
  try {
    ...
    if (count === 0) {
      ...
      const { error: insertError } = await supabase.from('conduct_catalog').insert(seed);
    ...
    if (/relation .* does not exist|table .* does not exist|"conduct_catalog"/.test(msg)) {
      console.warn('Tabla `conduct_catalog` no encontrada; se omite seed para este catálogo.');
      return false;
    }
    throw err;
  }
...
async function ensureCases() {
  ...
  if (count === 0) {
    ...
    const { error: insertError } = await supabase.from('cases').insert(seedCases);
    if (insertError) throw insertError;
  }
...
async function reportCounts() {
  ...
  const { count: casesCount, error: casesError } = await supabase.from('cases').select('id', { count: 'exact', head: true });
  ...
  console.log(`Filas en conduct_catalog: ${conductCount === null ? '(tabla no encontrada)' : conductCount}`);
